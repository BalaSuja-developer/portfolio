        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center space-x-9">
            {navItems.map((item) => (
              <button
                ...
                className="text-black hover:text-purple-600 font-medium transition-colors duration-200"
              >
                {item.label}
              </button>
            ))}
          ...
          <div className="md:hidden">
            <button
              ...
              className="text-black hover:text-purple-600 p-2"
            >
          ...
          <div className="md:hidden py-8 space-y-2">
            {navItems.map((item) => (
              <button
                key={item.sectionId} // ✅ Corrected
                onClick={() => scrollToSection(item.sectionId)} // ✅ Corrected
                className="block w-full text-left px-4 py-2 text-black hover:bg-purple-50 hover:text-purple-600 rounded-lg transition-colors"
              >
                {item.label}
              </button>
            ))}
